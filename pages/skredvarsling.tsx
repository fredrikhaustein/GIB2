...
const Skredvarsling = () => {
  ...
  return (
    <RecoilRoot>
      <div style={{ backgroundColor: "lightblue", height: "712px" }}>
        <a style={{ fontSize: "30px" }}>Varsling for: {mountainName}</a>
        {item.map((item) => (
          <ol key={item.RegId}>
            <a style={{ fontFamily: "fantasy" }}>{item.RegionName}</a>
            <ul>FARENIVÅ: {item.DangerLevel}</ul>
            <ul>FRA: {item.ValidFrom}</ul>
            <ul>TIL: {item.ValidTo}</ul>
            <ul>PUBLISERT: {item.PublishTime}</ul>
            <ul>VARSLING: {item.MainText}</ul>
          </ol>
        ))}
      </div>
    </RecoilRoot>
