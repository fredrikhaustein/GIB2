...
const MountainMap = ({ mapData }): JSX.Element => {
  ...
  const sortedDataArray = mapDataArray.sort((a, b) =>
  ...
  const [onClickFromSelection, setonClickFromSelection] = useState(false);
  ...
  const handleMountainChange = (e) => {
    ...
    setonClickFromSelection(!onClickFromSelection);
    setMountainNameRecoil(e.navn);
  };
    ...
    <GridRow>
      <GridColums>
        <select
          ...
          placeholder="Fjell"
        >
          {sortedDataArray.map((mountain: Mountain) => (
            <option
      ...
      >
        ...
        />
        {mountainStateLatlng && onClickFromSelection && (
